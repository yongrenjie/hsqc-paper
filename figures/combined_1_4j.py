...
fig, axs = pg.subplots(2, 3, figsize=(10, 8))

xlabels = [
    r"NOAH-2 $\rm S^{+}_{crk}C^{c}$ ($\Delta' = 1/(4J)$)",
    r"NOAH-2 $\rm S^{+}_{1}C^{c}$ ($\Delta' = 1/(4J)$)",
    r"NOAH-2 $\rm S^{+}_{2}C^{c}$ ($\Delta' = 1/(4J)$)",
    r"edited NOAH-2 $\rm S^{+}_{crk}C^{c}$ ($\Delta' = 1/(4J)$)",
    r"edited NOAH-2 $\rm S^{+}_{1}C^{c}$ ($\Delta' = 1/(4J)$)",
    r"edited NOAH-2 $\rm S^{+}_{2}C^{c}$ ($\Delta' = 1/(4J)$)",
]
ylabels = ([r"intensity vs NOAH-2 $\rm SC^{c}$"] * 3
           + [r"intensity vs edited NOAH-2 $\rm SC^{c}$"] * 3)
datasets = [
    [crk_s, crk_c],
    [spv1_s, spv1_c],
    [spv2_s, spv2_c],
...
editeds = [False] * 3 + [True] * 3
ref_datasets = [[sc_s, sc_c]] * 3 + [[edsc_s, edsc_c]] * 3

for ax, xlabel, ylabel, dataset, ref_dataset, edited in zip(axs.flat,
                                                            xlabels, ylabels,
