...
def hsqc_rel(ds, ref_ds, edited):
    ...
    return m["int"].tolist()
def cosy_rel(ds, ref_ds):
    df = Andro.cosy.rel_ints_df(dataset=ds, ref_dataset=ref_ds)
    m = df.mean()
    return m["int"].tolist()
def p_cosy_rel(ds, ref_ds):
    x = cosy_rel(ds, ref_ds)
    print(f"     &      &      & {x:.2f} \\\\")
def p_hsqc_cosy_rel(dss, ref_dss, edited):
    hsqcr = hsqc_rel(dss[0], ref_dss[0], edited=edited)
    cosyr = cosy_rel(dss[1], ref_dss[1])
    allr = [*hsqcr, cosyr]
    fs = lambda f: f"{f:.2f}"
    print(" & ".join(map(fs, allr)) + " \\\\")

p_cosy_rel(standalone_clipc, s[1])
...
p_cosy_rel(standalone_clipc, ed_s[1])
edited_comp = [ed_s[0], s[1]]
p_hsqc_cosy_rel(ed_s    , edited_comp, True)
p_hsqc_cosy_rel(ed_crk8 , edited_comp, True)
p_hsqc_cosy_rel(ed_spv18, edited_comp, True)
p_hsqc_cosy_rel(ed_spv28, edited_comp, True)
p_hsqc_cosy_rel(ed_crk4 , edited_comp, True)
p_hsqc_cosy_rel(ed_spv14, edited_comp, True)
p_hsqc_cosy_rel(ed_spv24, edited_comp, True)
