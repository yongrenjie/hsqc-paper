...
\hl{[Sorry, I was not very clear about this!]}
...
Since the seHSQC module does not preserve unused $\onejch$ magnetisation, the HSQC-TOCSY in a hypothetical \noahthree{Sp}{St}{Cc} supersequence will have greatly reduced sensitivity.
On the other hand, placing the HSQC-TOCSY sequence first allows the seHSQC module to be used in the other slot.
So, we will only consider here the permutations where the HSQC-TOCSY goes first.
...
It can be seen from \figref{stsc_comparisons} that the introduction of DIPSI-2 mixing leads to a very small drop ($< 10\%$) in the amount of bulk magnetisation preserved for the COSY module.
On the other hand, the HSQC (and seHSQC) sensitivites follow largely the same trend as before.
For values of $f$ above $0.7$ (where relatively little $\onejch$ magnetisation is preserved for these modules), the DIPSI-2 mixing helps to replenish some of this magnetisation.
