...
This is illustrated using a NOAH-2 \noahtwo{Sp}{Cc} (seHSQC + CLIP-COSY\autocite{Koos2016ACIE}) supersequence: \hl{the CLIP-COSY is used in this work as a convenient module for which peak intensities can be easily calculated, but the conclusions drawn here are fully applicable to any other homonuclear module that is used in its place.}
...
For \ce{CH2} and \ce{CH3} peaks, when the delay $\Delta'$ is set to $1/(8 \cdot \onejch)$ as done here, the gains that can be achieved via sensitivity enhancement are rather more modest (the case where $\Delta' = 1/(4\cdot\onejch)$ is explored in \cref{fig:1_4j_unedited}).\autocite{Schleucher1994JBNMR}
...
\hl{The benefits of the time savings afforded by NOAH supersequences are manifold.}
