...
\begin{figure}
    ...
    \caption{
        ...
        \textbf{(e)} Slices of the CLIP-COSY module in a NOAH-2 SCc supersequence (dashed line) and NOAH-2 SpCc (solid line) through $f_1 = \SI{1.36}{\ppm}$.
        \andro{}
    }
    \label{fig:spor_spv2}
\end{figure}

The solution to this is based on the simple observation that the bulk magnetisation in the seHSQC will be returned to $+z$ if the phase of the initial \proton{} \ang{90} pulse is changed by \ang{90} to $+y$.
...
On average, \ce{CH} and \ce{CH2} peaks have $1.54\times$ and $1.19\times$ increased sensitivity respectively relative to the NOAH HSQC module (\figref{spor_spv2}c--d).
However, a dramatic improvement is seen in the COSY module which follows.
In contrast to the CRK seHSQC, which completely destroys the requisite bulk magnetisation, the NOAH seHSQC preserves the majority of it, performing 92\% as well as the original HSQC module (\figref{spor_spv2}e).
...
This point is not relevant to the seHSQC, and here $k$-scaling on its own has only a tiny effect on peak height (and signal-to-noise), since any sensitivity gained from the extra transients is offset by the broadening (\figref{spv2_kscale}).
...
\begin{figure}
    ...
    \caption{
        ...
        \textbf{(b)} NOAH HSQC-TOCSY module (``St''), modified from the ASAP-HSQC-TOCSY.\autocite{Becker2019JMR}
        The gradients $g_3$ are \SI{1}{\ms} long, and are set to 19\% of the maximum gradient amplitude.
        \textbf{(c)} Overview of a NOAH-3 StSX or StSpX supersequence.
...
Furthermore, the sensitivity of the second HSQC can be further boosted by using the new seHSQC module in place of it (\figref{ssc_comparisons}).
...
\begin{figure}
    ...
    \caption{
        Example spectra obtained from the NOAH-4 SpnStSpCc supersequence.
...
For typical organic molecules, new supersequences such as the NOAH-4 StSpCT allow the rapid and complete collection of \ce{C-H} and \ce{H-H} correlations (\figref{stspct}).
