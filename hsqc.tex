...
\begin{center}
    \small
    \textbf{Keywords:} NMR spectroscopy $\cdot$ NOAH supersequences $\cdot$ Analytical methods $\cdot$ Sensitivity-enhanced HSQC $\cdot$ HSQC-TOCSY
\end{center}
% }}}1
\addtocontents{toc}{\protect\setcounter{tocdepth}{0}}  % disable TOC
...
\subsection{\texorpdfstring{\proton{}--\carbon{}}{1H-13C} sensitivity-enhanced HSQC}
...
\begin{figure}
    ...
    \caption{
        ...
        \textbf{\subref{fig:pprogs_spv2}} Version 2 of the NOAH seHSQC module (``ZIP-seHSQC''), abbreviated as ``\noahSpb{}''.
...
\begin{figure}
    ...
    \caption{
        ...
        \textbf{\subref{fig:sehsqc_comp_crk}} Using the original CRK seHSQC (\cref{fig:pprogs_crk}), which leads to severely reduced COSY intensities.
...
On the other hand, the second (\cref{fig:pprogs_spv2}, ``\noahSpa'' or ``ZIP-seHSQC'') actively differentiates the two components at the start of the sequence by prepending a double heteronuclear spin echo, a strategy recently reported by Hansen \textit{et al.}\autocite{Hansen2021AC}
...
For example, with the present sample, when the delay $\Delta'$ is set to $1/(8\cdot\onejch)$ as was done here,\autocite{Schleucher1994JBNMR} the \noahSpa{} and \noahSpb{} modules yield on average a $1.29\times$ and $1.54\times$ boost for \ce{CH} peaks, which are smaller than the $1.80\times$ increase seen with the CRK seHSQC (the case where $\Delta' = 1/(4\cdot\onejch)$ is explored in \cref{fig:1_4j_unedited,fig:1_4j_edited}).
...
Nevertheless, it should be emphasised that the \noahSpb{} module in particular still provides clear sensitivity gains over the reference ``gold-standard'' NOAH HSQC module, especially for \ce{CH} and \ce{CH2} peaks (\cref{fig:sehsqc_comp_ex_ch,fig:sehsqc_comp_ex_ch2}) which are typically less intense.
...
\subsection{\texorpdfstring{\proton{}--\nitrogen{}}{1H-15N} sensitivity-enhanced HSQC}
...
\begin{figure}
    ...
    \caption{
        ...
        \textbf{\subref{fig:15n_spv2vsm_hmqc}} X = \noahM{}, i.e.\ \nitrogen{} HMQC module.
        \textbf{\subref{fig:15n_spv2vsm_spv2}} X = \noahSpn{}, i.e.\ \nitrogen{} seHSQC module.
...
The delays in the HSQC sequences were optimised for $\onejch = \SI{145}{\Hz}$ and $\onejnh = \SI{90}{\Hz}$ respectively, and the CLIP-COSY mixing delay (denoted by $\tau$ in \cref{fig:sehsqc_comp_pprog_clipcosy}) was set to \SI{8.33}{\ms}, corresponding to a nominal $\jhh$ value of \SI{30}{Hz}.
