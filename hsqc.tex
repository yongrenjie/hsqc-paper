...
For \ce{CH2} and \ce{CH3} peaks, when the delay $\Delta'$ is set to $1/(8 \cdot \onejch)$ as done here, the gains that can be achieved via sensitivity enhancement are rather more modest (the case where $\Delta' = 1/(4\cdot\onejch)$ is explored in \cref{fig:combined_1_4j}).\autocite{Schleucher1994JBNMR}
...
Multiplicity editing\autocite{Parella1997JMR} can be easily incorporated into both NOAH seHSQC sequences (\cref{fig:mult_edit}) by expanding the spin echo immediately following $t_1$.
...
Next, we note that the HSQC module (though not the new seHSQC modules) allows an arbitrary amount of \magn{\ce{C}} magnetisation to be excited, with the remainder returned to $+z$.\autocite{Schulze-Sunninghausen2014JACS, Schulze-Sunninghausen2017JMR, Koos2019JMR}
...
Secondly, since each NOAH module is independently executed, the NOAH approach allows multiplicity editing to be enabled for only the HSQC and not the HSQC-TOCSY, where accidental overlap may lead to crosspeaks being lost unexpectedly.
